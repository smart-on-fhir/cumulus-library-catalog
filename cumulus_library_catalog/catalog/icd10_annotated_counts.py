...
class IcdCountsBuilder(cumulus_library.CountsBuilder):
    ...
    def get_icd10_diagnosis_query(self, table_name, depth, min_subject: int | None = None):
        ...
        """
        potential_cols = [
            (("chapter_code", None),("chapter_str", None)),
            (("block_code", None),("block_str", None)),
            (("category_code", None),("category_str", None)),
            (("subcategory_1_code", None),("subcategory_1_str", None)),
            (("subcategory_2_code", None),("subcategory_2_str", None)),
            (("subcategory_3_code", None),("subcategory_3_str", None)),
            (("extension_code", None),("extension_str", None)),
        ]
        if depth > len(potential_cols) | depth <= 0:
        ...
        if depth != 0:
            annotation_cols.append(potential_cols[(depth - 1)][0])
            annotation_cols.append(potential_cols[(depth - 1)][1])
        annotation_cols.append(potential_cols[depth][0])
        annotation_cols.append(potential_cols[depth][1])
        target_col = potential_cols[depth][0][0]
    ...
    def prepare_queries(
        ...
        **kwargs,
        ...
        for depth,table_name in enumerate(
            ["chapter","block","category","subcategory_1","subcategory_2","subcategory_3","extension"]
        ):
            self.queries.append(
                self.get_icd10_diagnosis_query(
                    f"catalog__count_icd10_{table_name}", depth, min_subject
                )
            )
